...
**Process**:
1. Checks for existing booking (by phone/email)
2. Creates Google Calendar event
3. Stores booking in `web_sessions` table
4. Updates `all_leads` if linked
...
3. Calls `/api/calendar/book`
4. Stores booking in `web_sessions`
...
- `checkExistingBooking()` - Check if user already has booking
...
11. POST /api/calendar/book
    ...
    - Stores booking in web_sessions
    - Updates all_leads if linked
    ↓
12. Conversation continues
    - Periodically: POST /api/chat/summarize
    - Compresses conversation
    - Updates conversation_summary in web_sessions
    ↓
13. Dashboard reads from unified_leads view
    - Real-time updates via Supabase Realtime
