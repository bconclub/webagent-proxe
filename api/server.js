...
// Note: Static file serving removed - frontend is now deployed separately on Vercel
// API server only handles /api/* endpoints
...
if (!claudeApiKey) {
  ...
  console.error('   The chatbot will NOT be able to generate responses without this key');
    console.error('   Please add CLAUDE_API_KEY to api/.env file');
...
// No catch-all routes needed - only handle /api/* endpoints
